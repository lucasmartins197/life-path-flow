...
import {
  ...
  CheckCircle, Lock, RefreshCw, PlayCircle, CheckSquare,
} from "lucide-react";
...
const STEPS: Record<number, StepContent> = {
  ...
  12: {
    ...
    activity: "Publique sua história de conquista em Histórias que Conectam e compartilhe o app Stake Real com alguém que precisa. Você completa sua jornada ajudando outros a começar a deles.",
      ...
      <div className="max-w-2xl mx-auto px-5 pt-5 space-y-5">
        ...
        {STEP_AUDIO[stepNumber] && (
          <div className="rounded-2xl p-4 shadow-md" style={{ background: "linear-gradient(135deg, #1B4332, #2D6A4F)" }}>
            <div className="flex items-center gap-2 mb-3 text-white">
              ...
              {audioPlayed && (
                <span className="text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1" style={{ background: "rgba(201,168,76,0.2)", color: "#E8D590" }}>
        ...
        {audioPlayed && quiz && !isCompleted && (
          <div className="rounded-2xl p-5 bg-card border border-border shadow-sm">
            <div className="flex items-center gap-2 mb-4">
              ...
              {quizPassed && (
                <span className="ml-auto text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1" style={{ background: "#D1FAE5", color: "#065F46" }}>
              ...
              <Button
                onClick={() => {
                  ...
                  if (correct >= 2) {
                    ...
                    toast({ variant: "destructive", title: `❌ ${correct}/3 corretas. Releia o conteúdo e tente novamente.` });
                    setTimeout(() => {
                      setQuizAnswers({});
        ...
        <div className="rounded-2xl p-5 bg-card border border-border shadow-sm">
          ...
          <p className="text-foreground text-sm mb-3 leading-relaxed">
            {quiz?.depositoPrompt || step.question}
          </p>
          <Textarea
            value={resposta}
        ...
        {!isCompleted && (
          <div className="rounded-2xl p-4 bg-card border border-border">
            <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-3">Para concluir este passo:</p>
            <div className="space-y-2">
              <div className={`flex items-center gap-2 text-sm ${audioPlayed ? "text-green-600" : "text-muted-foreground"}`}>
                {audioPlayed ? <CheckCircle className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
                Ouvir o áudio
              </div>
              <div className={`flex items-center gap-2 text-sm ${quizPassed ? "text-green-600" : "text-muted-foreground"}`}>
                {quizPassed ? <CheckCircle className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
                Responder o questionário (2/3 corretas)
              </div>
              <div className={`flex items-center gap-2 text-sm ${resposta.trim().length >= 50 ? "text-green-600" : "text-muted-foreground"}`}>
                {resposta.trim().length >= 50 ? <CheckCircle className="h-4 w-4" /> : <Lock className="h-4 w-4" />}
                Escrever depoimento (mín. 50 caracteres)
              </div>
              <div className={`flex items-center gap-2 text-sm ${stepTaskDone || isAdmin ? "text-green-600" : "text-muted-foreground"}`}>
        ...
        <Button
          ...
          disabled={completing || !resposta.trim() || resposta.trim().length < 50 || (!stepTaskDone && !isAdmin) || (!quizPassed && !isCompleted)}
